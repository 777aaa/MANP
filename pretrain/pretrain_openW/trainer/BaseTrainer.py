...
class BaseTrainer(object):
    def __init__(self, args, dataset_trainer):
        ...
        fusion_raw_model = state_dict['fusion_params']
        ...
        if 'ImageNet' in args.dataset:
            self.semantic = torch.load('/root/wjg/jbw/SEMOP/SemFew/semantic/imagenet_semantic_{}_{}.pth'.format(args.mode, args.text_type))['semantic_feature']
        else:
            self.semantic = torch.load('/root/wjg/jbw/SEMOP/SemFew/semantic/cifar100_semantic_{}_{}.pth'.format(args.mode, args.text_type))['semantic_feature']
        self.semantic = {k: v.float() for k, v in self.semantic.items()}
        print("Loaded  semantic")
        ################################################
        options = vars(args)
    ...
    #####################################################################
    def RPL_train_epoch(self, epoch, train_loader, model,fusion, criterion_cls, criterion_open, optimizer, args, optimizer2=None):
        self.model.eval()
        self.fusion.eval()
    ...
    def train(self, eval_loader=None):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            train_msg = self.RPL_train_epoch(epoch, self.train_loader, self.model,self.fusion, self.GCPLcriterion, self.RPLcriterion, self.RPL_GCPL_optimizer, self.args, self.optimizer)

            #evaluate
            if eval_loader is not None and (epoch % 5 == 0 or epoch >= 55):
                start = time.time()
                eval_1shot_loader,eval_5shot_loader = eval_loader
                meta_1shot_acc, meta_1shot_std= meta_evaluation(self.model, eval_1shot_loader, self.fusion, self.semantic, self.args.k1)
                meta_5shot_acc, meta_5shot_std = meta_evaluation(self.model, eval_5shot_loader, self.fusion, self.semantic, self.args.k5, shot_5=True)
