...
parser.add_argument('--k', type=float, default=0.19)
...
def eval(args, model, meta_test_loader, config):
    ...
    if 'ImageNet' in args.dataset:
            #/root/data/jbw/SemFew/semantic
        semantic = torch.load('/root/wjg/jbw/SEMOP/SemFew/semantic/imagenet_semantic_clip_gpt.pth')['semantic_feature']
    else:
        semantic = torch.load('/root/wjg/jbw/SEMOP/SemFew/semantic/cifar100_semantic_clip_gpt.pth')['semantic_feature']
    if args.featype == 'OpenMeta':
        config = run_test_fsl(model, meta_test_loader,config,semantic)
...
if __name__ == "__main__":
    ...
    log_file = 'test_%s_%s_%s.log' % (args.comment, args.dataset,args.n_shots)
    ...
    if args.featype == 'OpenMeta':
        model = FeatureNet(args, args.restype, n_cls, (cls_params, meta_test_loader.dataset.vector_array),fusion_params)
    else:
        model = GFeatureNet(args, args.restype, n_cls, (cls_params, meta_test_loader.dataset.vector_array),fusion_params)
