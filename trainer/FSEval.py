...
def run_test_fsl(net, openloader,config,semantic,encoder=None, generator=None, n_ways=5,n_shots=1,scale=4):
    net = net.eval()
    ...
    with tqdm(openloader, total=len(openloader), leave=False) as pbar:
        ...
        for idx, data in enumerate(pbar):
            feats, labels, probs = compute_feats(net, data,semantic)
...
def compute_feats(net, data,semantic):
    with torch.no_grad():
        # Data Preparation
        support_data, support_label, query_data, query_label, suppopen_data, suppopen_label, openset_data, openset_label, supp_idx, open_idx,idx_to_class,min_label = data
        #print(support_data.shape)#torch.Size([1, 125, 3, 84, 84])
        #print(query_data.shape)#torch.Size([1, 75, 3, 84, 84])
        #print(suppopen_data.shape)#torch.Size([1, 125, 3, 84, 84])
        #print(openset_data.shape)#torch.Size([1, 75, 3, 84, 84])
        idx_to_class = {k: v[0] for k, v in idx_to_class.items()}
        #print('test-support_label',support_label)
        #print('test-supp_idx',supp_idx)
        #print("test-idx_to_class",idx_to_class)
        ''' {80: 'n02110341', 81: 'n01930112', 82: 'n02219486', 83: 'n02443484', 92: 'n02871525', 84: 'n01981276', 85: 'n02129165', 87: 'n02099601', 88: 'n03775546', 90: 'n02116738', 91: 'n03146219', 86: 'n04522168', 89: 'n02110063', 98: 'n04418357', 94: 'n03544143', 95: 'n03272010', 96: 'n07613480', 97: 'n04146614', 93: 'n03127925', 99: 'n04149813'}'''
        #print("eval",supp_idx)#tensor([[18,  1, 19,  8, 10]])
        supp_idx_s = supp_idx.squeeze()
        supp_idx_s = supp_idx_s+min_label
        open_idx_s = open_idx.squeeze()
        open_idx_s = open_idx_s + min_label
        #print("eval1",supp_idx_s)#tensor([98, 81, 99, 88, 90])
        supp_idx_s = supp_idx_s.repeat_interleave(5)
        #print("eval1",supp_idx_s)#tensor([98, 98, 98, 98, 98, 81, 81, 81, 81, 81, 99, 99, 99, 99, 99, 88, 88, 88,88, 88, 90, 90, 90, 90, 90])
        if net.n_shots ==5:
            supp_idx_s=supp_idx_s.repeat_interleave(5)
            open_idx_s=open_idx_s.repeat_interleave(5)
            #print("test",supp_idx_s)
            '''tensor([98, 98, 98, 98, 98, 98, 98, 98, 98, 98, 98, 98, 98, 98, 98, 98, 98, 98,
                    98, 98, 98, 98, 98, 98, 98, 81, 81, 81, 81, 81, 81, 81, 81, 81, 81, 81,
                    81, 81, 81, 81, 81, 81, 81, 81, 81, 81, 81, 81, 81, 81, 99, 99, 99, 99,
                    99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99,
                    99, 99, 99, 88, 88, 88, 88, 88, 88, 88, 88, 88, 88, 88, 88, 88, 88, 88,
                    88, 88, 88, 88, 88, 88, 88, 88, 88, 88, 90, 90, 90, 90, 90, 90, 90, 90,
                    90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90])'
                    '''
        #print('test-supp_idx_s',supp_idx_s)
        text_feature = torch.stack([semantic[idx_to_class[l.item()]] for l in supp_idx_s])
        open_text_feature = torch.stack([semantic[idx_to_class[l.item()]] for l in open_idx_s])
        ...
        open_text_feature = open_text_feature.unsqueeze(0)
        ...
        features, cls_protos, cosine_probs= net(text_feature,open_text_feature,the_img,the_label,the_conj,test=True)
