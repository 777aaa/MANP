...
class Classifier(nn.Module):
    ...
    def forward(self,fusion,text_feature,features, cls_ids, test=False,mixup_part = None):
        ...
        #support_feat_fusion = torch.mean(support_feat, dim=2)
        #print("class-support_feat",support_feat.shape)#torch.Size([2, 5, 1, 640]) torch.Size([2, 5, 5, 640])
        #print("class-text_feature",text_feature.shape)#torch.Size([2, 5, 1, 512]) torch.Size([2, 5, 5, 640])
        #print(type(text_feature)) 
        support_feat_fusion,logit = fusion(text_feature, support_feat)#
        #recon_loss = F.l1_loss(support_feat_fusion, support_feat)
        #先对每个样本特征融合增强后再取均值得到原型
        #print("class-proto",support_feat.shape)#torch.Size([2, 5, 1, 640]) torch.Size([2, 5, 5, 640])
        #print(type(support_feat)) 
        #print(type(support_feat_fusion)) 
        support_feat_fusion = self.k * support_feat + (1 - self.k) * support_feat_fusion
        
        support_feat_fusion= torch.mean(support_feat_fusion, dim=2)
        #print("class-support_feat_fusion",support_feat_fusion.shape) #torch.Size([2, 5, 640])
        supp_protos =support_feat_fusion
        #消融-原型无多模态
        #supp_protos =  torch.mean(support_feat, dim=2)
        sample_loss = 0
        #stage2
        fakeclass_protos, recip_unit = self.open_generator(supp_protos, base_weights, support_seman=support_seman,base_seman=base_seman,base_open_weights = base_open_weights) # generate np for each novel class (recip point), and generate task's np.
...
class OpenSetGenerater(nn.Module):
    ...
    def forward(self, support_center, base_weights, support_seman=None, base_seman=None,base_open_weights = None,support_attn=None):
        ...
        #support_center = self.self_attn(support_center,support_center,support_center)
        base_mem_vis = base_weights
        support_seman = None
        base_seman = None

        #output1, attcoef, attn_score, value = self.calibrator(support_center, base_weights, base_weights, support_seman, base_seman)  ## bs*nway*nbase
        ######### negative_proto
